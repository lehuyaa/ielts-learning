  ...
  return (
    <div className="min-h-screen bg-[#f8f8ff] text-[#10111f]">
      <header className="border-b border-[#e6e6f3] bg-white">
        <div className="mx-auto flex min-h-[72px] max-w-[1200px] items-center justify-between gap-4 px-4 md:px-6">
...
function WordHero({ vocabulary }: { vocabulary: VocabularyDetailViewModel }) {
  return (
    <section className="rounded-[28px] bg-gradient-to-br from-[#6258f6] to-[#8318e8] p-6 text-white shadow-sm md:p-7">
      <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-4xl font-bold tracking-normal md:text-5xl">
            {vocabulary.word}
          </h2>
          <p className="mt-3 font-mono text-lg font-semibold text-white/75">
            {vocabulary.ipa}
            <span className="ml-4 font-sans text-white/80">
              {vocabulary.partOfSpeech}
            </span>
          </p>
          <p className="mt-5 flex items-center gap-3 text-base font-medium text-white/80">
        ...
        <div className="grid justify-items-start gap-4 md:justify-items-end">
          <Button className="h-10 rounded-full bg-white/20 px-5 text-sm text-white hover:bg-white/25">
...
function VocabularyTabs({ activeTab, onChange }: VocabularyTabsProps) {
  return (
    <div className="flex overflow-x-auto rounded-[22px] bg-[#e9e9f3] p-1">
      {tabs.map((tab) => (
        <button
          className={cn(
            'min-w-fit cursor-pointer rounded-[18px] px-5 py-2.5 text-base font-bold transition-colors',
            activeTab === tab
      ...
      <div className="mt-5 grid gap-4">
        {vocabulary.examples.map((example) => (
          <ContentCard eyebrow="IELTS Example" key={example.sentence}>
            <blockquote className="text-lg font-medium text-[#30348e]">
              "{example.sentence}"
            </blockquote>
        ...
        <div className="flex flex-wrap gap-3">
          {vocabulary.relatedForms.length > 0 ? (
            vocabulary.relatedForms.map((form) => (
              <span
                className="rounded-full border border-[#dde3ff] bg-[#f0efff] px-3 py-2 text-sm font-bold text-primary"
...
function ContentCard({ eyebrow, children }: ContentCardProps) {
  return (
    <section className="rounded-2xl border border-[#e6e6f3] bg-white p-5 shadow-sm">
      ...
      <div className="mt-3 text-[#10111f]">{children}</div>
    </section>
  )
}
  ...
  return (
    <ContentCard eyebrow={eyebrow}>
      <div className="flex flex-wrap gap-3">
        {tokens.map((token) => (
          <span
            className="rounded-full bg-[#f0efff] px-3 py-2 text-sm font-bold text-primary"
            key={token}
          >
            {token}
          </span>
    ...
    <aside className="grid gap-5 lg:sticky lg:top-6">
      <section className="rounded-2xl border border-[#e6e6f3] bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3">
          <Button className="h-12 justify-between rounded-2xl bg-[#eff1ff] px-4 text-base font-bold text-primary hover:bg-[#e6e8ff]">
          ...
          <Button className="h-12 justify-between rounded-2xl bg-[#e8fff3] px-4 text-base font-bold text-[#208b5a] hover:bg-[#dcfce7]">
            <span className="inline-flex items-center gap-3">
      ...
      <section className="rounded-2xl border border-[#e6e6f3] bg-white p-5 shadow-sm">
        <h2 className="text-lg font-bold tracking-normal">Word Stats</h2>
        <div className="mt-5 grid gap-3">
      ...
      <section className="rounded-2xl bg-gradient-to-br from-[#45c486] to-[#2e9f83] p-5 text-white shadow-sm">
        <p className="text-sm font-bold text-white/75">Mastery Score</p>
